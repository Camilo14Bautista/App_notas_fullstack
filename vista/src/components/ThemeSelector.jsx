...
function ThemeSelector({ showButtons = true }) {
  ...
  const themes = [
    {
      ...
      namepr: "Tema claro",
      color:
        "hover:text-yellow-400 dark:hover:text-yellow-400  hover:bg-yellow-200 dark:hover:bg-yellow-200",
  ...
  return (
    <Menu as="div" className="relative inline-block text-left z-50">
      ...
      {showButtons && (
        <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-800/80 backdrop-blur-sm">
          <div className="px-1 py-1">
            {themes.map((theme) => (
              <Menu.Item key={theme.name}>
                {() => (
                  <button
                    className={`${theme.color} group flex w-full items-center rounded-md px-2 py-2 text-sm dark:text-white text-gray-700`}
      ...
      <Tooltip id="my-tooltipt"></Tooltip>
    </Menu>
  );
}
